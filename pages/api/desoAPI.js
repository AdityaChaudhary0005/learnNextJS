...
const DEFAULT_BACKEND_URL = "https://mintedtweets.cordify.app";
...
class DesoApi {
  ...
  async claimNft(ENC, publicKey, screenName, postHashHex) {
    const path = DEFAULT_BACKEND_URL + "/transfer-NFT";
    const data = {
      ENC: ENC,
      publicKey: publicKey,
  ...
  async getUserContentByID(userID) {
    const path = DEFAULT_BACKEND_URL + "/get-content-by-userID";
    const data = {
      userID: userID,
    };
    try {
  ...
  async getUserContentByTag(tag) {

    const path = DEFAULT_BACKEND_URL + "/get-content-by-tag";
    const data = {
      tag: tag,
    };
    try {
  ...
  async getTwitterLoginURL() {
    const path = "/loginWithTwitter";
    try {
      const result = await this.getClient().get(DEFAULT_BACKEND_URL + path);
      return result.data;
    } catch (error) {
      console.log(error);
      return null;
  ...
  async updateProfile(newDescription, newProfilePic, FR, newUsername, UpdaterPublicKey){
